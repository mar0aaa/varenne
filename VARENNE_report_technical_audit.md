...
1. **Median size equation `X50`** — `kco_model.py:729-783`
...
6. **Rock factor `A` and blastability index `BI` conceptual form**
   - `A = 0.06 BI` is correct (Cunningham 1987; `kco_model.py:522-560`).
   - `RDI = 0.025 rho - 50` is correct (`kco_model.py:394-416`).
   - `HF` with `E < 50 GPa -> E/3` and `E >= 50 GPa -> UCS/5` is correct for `E=60`, `UCS=200` giving `HF=40` (`kco_model.py:419-447`).

7. **Joint factor `JF = JPS + JPA`** — `kco_model.py:363-388`
   - This is the Cunningham (1987) baseline without joint condition factor. Correct.

8. **JPS thresholds** — `kco_model.py:308-360`
   - `JPS = 10` if `Sj < 0.1 m`; `20` if `0.1 <= Sj < 0.3 m`; `50` if `0.3 <= Sj < 0.95 sqrt(BS)`; `80` if `Sj >= 0.95 sqrt(BS)`.
   - For `B=3.4 m`, `S=4.1 m`, `0.95 sqrt(BS) = 3.55 m`. Correct.
...
10. **`S_j^* = V^{1/3}` equivalent-cube conversion** — `kco_model.py:789-806`, `kco_star_model.py:118-172`
...
15. **Collapse of 12 classes into 3 distinct `X50` values**
    - The `VARENNE_kco_star_section3_table.csv` shows three distinct `X50` values: 178.94 mm (JPS=10), 197.29 mm (JPS=20), 252.35 mm (JPS=50). This is a direct consequence of JPS being a step function. Correct.

16. **KCO/KCO* are much finer than WipFrag**
    - WipFrag `D50` ~540.75 mm; KCO `X50` = 252.35 mm; KCO* `X50*` ≈ 245 mm. Both models are clearly finer than the measurement. Correct interpretation.

17. **`n_s = 1` and `g(n) = 1` are the baseline, no-shift, no-timing-scatter assumptions** — `kco_model.py:677-706`
...
1. **Blastability-index equation in the paper is currently written as `BI = RMD + JF + RDI + HF`. This is incorrect and double-counts `JF`.**
   - In the current code, `BI = RMD + RDI + HF` (`kco_model.py:496-519`), and for jointed rock `RMD = JF` (`kco_model.py:453-493`) where `JF = JPS + JPA`.
...
1. **Exact KCO* `X50* ≈ 245.56 mm` for `UCS=200`**
   - The `VARENNE_kco_star_section3_table.csv` was run with `UCS=200` and gives class `X50` values of 178.94, 197.29, and 252.35 mm, with weights 2.64 %, 7.69 %, and 89.67 %.
   - The exact mixture `X50*` (solution of `sum_i w_i P_i(x) = 50 %`) should be read from `KCOStarResult.percentiles()` with the same `UCS=200` design. The report value 245.56 mm is plausible but should be explicitly reproduced from the code and the `xmax_block_percentile` (95) and `class_method` (log_bins_p1_p99_tails) stated.
...
3. **`Xmax` for the comparison curves**
   - The `Section 3` table gives `Xmax = 1813.48 mm` for every class, governed by the in-situ block size. The comparison-plot `Xmax` should match this value if it uses the same `xmax_block_percentile=95` and `equivalent_cube` method. Verify that the paper states this explicitly.
...
5. **`Xmax_block_percentile` and `block_size_method` for `Xmax`**
   - These are user choices. The paper should report them so the figure/table is reproducible. Recommended: "`Xmax` is the P95 equivalent-cube side of the DFN block-volume distribution, capped by `B` and `S`."
...
*(Source: `kco_model.py:496-519`, `rock_mass_description`, `blastability_index`; Ouchterlony 2005, eq. (11e).)*
...
|---|---|---|
| Cunningham (1987) | `A = 0.06 BI`; `JF = JPS + JPA`; JPS thresholds | `kco_model.py:522-560`, `363-388`, `308-360` |
| Cunningham (2005) | Uniformity index `n` (with 2019 correction) | `kco_model.py:582-666` |
| Ouchterlony (2005) | `X50` eq. (11b), Swebrec eq. (11a), `b` eq. (11c), `Xmax` eq. (11d), `RMD` eq. (11e) | `kco_model.py:729-783`, `1014-1053`, `976-1008`, `933-970`, `453-493` |
| Ouchterlony & Sanchidrián (2019) | Corrected `n` with `(A/6)^0.3` | `kco_model.py:582-666` |
| Lilly (1986) / Cunningham (1987) | `HF` with `E < 50` vs `E >= 50` | `kco_model.py:419-447` |
